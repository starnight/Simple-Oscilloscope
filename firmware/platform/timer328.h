...
#define Timer2EnableOCInterrupt()	(TIMSK2 |= (1 << OCIE2A))
/* Disable timer#2 output compare match interrupt. */
#define Timer2DisableOCInterrupt()	(TIMSK2 &= ~(1 << OCIE2A))
/* Initial timer#2 with CTC mode. */
#define Timer2Init() ({ \
	/* Set timer#2 CTC and OC2 disconnected mode.*/ \
	TCCR2A = (1 << WGM21) | (0 << WGM20) | (0 << COM2A0); \
...
#define Timer2SetTop(top)	(OCR2A = top - 1)
...
#define Timer2CompareEvent()	ISR(TIMER2_COMPA_vect)
